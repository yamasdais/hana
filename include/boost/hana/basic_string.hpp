...
BOOST_HANA_NAMESPACE_BEGIN
    ...
    namespace basic_string_detail {
        ...
        constexpr CharTo cast_as_unsigned(CharFrom value) {
        ...
        constexpr basic_string<CharT, S::cast(S::get()[N])...>
        ...
        constexpr decltype(auto) prepare(S s) {
            return prepare_impl<CharT>(
                ...
                std::make_index_sequence<
                    sizeof(S::get())/sizeof(typename S::from_type) - 1
                >{}
            );
        }
    }
    ...
    (::boost::hana::basic_string_detail::prepare<                   \
        std::decay_t<decltype(*s)>>([]{                             \
            struct tmp {                                            \
                ...
                static constexpr from_type cast(from_type v) {      \
                    return v;                                       \
                }                                                   \
            };                                                      \
            return tmp{};                                           \
...
#define BOOST_HANA_BASIC_STRING(type, s)                         \
    (::boost::hana::basic_string_detail::prepare<type>([]{       \
        struct tmp {                                             \
            using from_type = std::decay_t<decltype(*s)>;        \
            static constexpr decltype(auto) get() { return s; }  \
            static constexpr type cast(from_type v) {            \
                return ::boost::hana::basic_string_detail::      \
                           cast_as_unsigned<type>(v);            \
            }                                                    \
        };                                                       \
        return tmp{};                                            \
    }()))
